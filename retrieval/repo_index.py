...
def clean_shadow_tags(raw: str) -> str:
    ...
    # 去掉一些常见“解释型废话”
    bad_prefixes = ["无法确定", "无法识别", "请提供", "以下是", "根据文本", "关键词如下", "核心关键词如下", ]
...
def read_changed_file(notes_dir: Path, relative_path: str, logger):
    ...
    return {"path": relative_path, "doc": content, "file_time": mtime, "file_size": stat.st_size,
...
def scan_repository(notes_dir: Path, logger):
    ...
    for file in all_files:
        ...
        entries.append({"path": relative_path, "file_time": mtime, "fingerprint": fingerprint, "size_kb": size_kb, })
    ...
    return {"entries": entries, "paths": [e["path"] for e in entries], "file_times": [e["file_time"] for e in entries],
        "file_info_list": file_info_list, "all_files": all_files, "earliest_note": earliest_note,
        "latest_note": latest_note, "notes_dir": notes_dir, }
...
def _extract_shadow_tags(doc: str, path: str, logger, ollama_api_url: str, ollama_model: str) -> str:
    ...
    try:
        summary_prompt = (f"请提取以下私人笔记片段的 5-8 个最核心搜索关键词。\n"
                          f"【严格分类指令】：\n"
...
def load_or_build_embeddings(scanned, cache_file: Path, model_emb, logger, ollama_api_url: str, ollama_model: str):
    ...
    current_manifest = {path: fp for path, fp in zip(current_paths, current_file_fingerprints)}
    ...
    for path in unchanged_paths:
        ...
        if (isinstance(doc_entry, dict) and isinstance(chunk_entry, dict) and doc_entry.get("fingerprint") ==
                path_to_fp[path] and chunk_entry.get("fingerprint") == path_to_fp[path]):
    ...
    for path in changed_paths:
        ...
        file_chunk_meta = [{"path": path, "chunk_id": idx, "start": ch["start"], "end": ch["end"], } for idx, ch in
            enumerate(chunks)]
        ...
        new_doc_cache[path] = {"fingerprint": fingerprint, "doc": doc, "file_time": file_time.timestamp(),
            "file_size": int(file_record["file_size"]), "file_info": file_record["file_info"],
            "shadow_tags": shadow_tags, "embedding": np.asarray(doc_embedding), }

        new_chunk_cache[path] = {"fingerprint": fingerprint, "chunk_texts": file_chunk_texts,
            "chunk_meta": file_chunk_meta, "chunk_file_time": file_time.timestamp(),
    ...
    for path in current_paths:
        ...
        doc_records.append({"path": path, "shadow_tags": doc_entry.get("shadow_tags", "") or "",
            "file_time": datetime.datetime.fromtimestamp(float(doc_entry["file_time"])),
            "file_size": int(doc_entry.get("file_size", 0) or 0), "file_info": doc_entry["file_info"], })
    ...
    np.savez(cache_file, manifest=np.array(current_manifest, dtype=object),
        doc_cache=np.array(new_doc_cache, dtype=object), chunk_cache=np.array(new_chunk_cache, dtype=object), )
    ...
    return RepoState(docs=docs, doc_records=doc_records, paths=final_paths, file_times=file_times,
                     file_info_list=file_info_list, chunk_texts=chunk_texts, chunk_paths=chunk_paths, chunk_meta=chunk_meta,
        chunk_file_times=chunk_file_times, all_files=scanned["all_files"], embeddings=embeddings,
        chunk_embeddings=chunk_embeddings, earliest_note=scanned["earliest_note"], latest_note=scanned["latest_note"], )
